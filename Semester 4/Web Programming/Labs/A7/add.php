...
if (isset($_POST['group'])) {
	...
	if (is_numeric($group) && is_string($course)) {
...
if (isset($_POST['submit'])) {
	...
	if (is_numeric($studentId) && is_numeric($grade) && is_numeric($courseId)) {
		$con = mysqli_connect("127.0.0.1", "root", "", "students");
...
<body>
	<div>
		...
		if (is_numeric($group) && is_string($course) && is_numeric($courseId)) {
		...
		<ul class="pagination">
			...
			<li class="<?php if ($pageno1 <= 1) {
							echo 'disabled';
						} ?>">
				<a href="<?php if ($pageno1 <= 1) {
								echo '#';
							} else {
								echo "?pageno1=" . ($pageno1 - 1);
							} ?>">Prev</a>
			</li>&emsp;
			<li class="<?php if ($pageno1 >= $total_pages) {
							echo 'disabled';
						} ?>">
				<a href="<?php if ($pageno1 >= $total_pages) {
								echo '#';
							} else {
								echo "?pageno1=" . ($pageno1 + 1);
							} ?>">Next</a>
