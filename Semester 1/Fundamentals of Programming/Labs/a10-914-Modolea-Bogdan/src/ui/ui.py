                ...
                if line == "1":
                    ...
                    if now == "1":
                        ...
                        if new_number_person == len(my_persons):
                            ...
                            check = self.__services.check_update(now_act, curr_id, my_persons, new_date, new_time,
                                                                 new_description)
                            if check == True:
                                print("Sorry! A person is already busy and cannot participate at this activity")
                        else:
                    ...
                    if cmd == "1":
                        ...
                        for each_activity in my_act:
                            if each_activity[1] > 1:
                                ...
                                print(
                                    "The date " + each_activity[0] + " has " + str(
                                        each_activity[1]) + " activity planned.")
                    elif cmd == "3":
                        name = input("Enter a name: ")
                        my_persons = self.__services.get_all_persons()
                        my_act = self.__services.get_activities_by_name(name, my_persons)
                        for each_activity in my_act:
                            print(name + " does activity with id " + str(
                                each_activity.id) + ", at the date and time " + each_activity.date + ", " + each_activity.time)
                elif line == "7":
