...
public class MyScanner {
    ...
    public void scan() {
        ...
        for (Pair<String, Integer> tokenPair : tokens) {
            ...
            } else if (Pattern.compile(CONST_REGEX).matcher(token).matches() || new FiniteAutomaton("tokens/constants.in").checkSequence(token)) {
                st.addSymbol(token);
...
//            } else if (Pattern.compile(IDENTIFIER_REGEX).matcher(token).matches()) {
            } else if (new FiniteAutomaton("tokens/identifiers.in").checkSequence(token)) {
                st.addSymbol(token);
